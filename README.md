...
| 이름  | 역할 | 담당 | GitHub |
|:---:|:---:|:---|:---:|
| **이&#8288;재&#8288;희** | PM · AWS · CI/CD · Android | AWS ECS 기반 운영 인프라 설계 · 배포<br>GitHub Actions CI/CD 및 Smoke Test 구현<br>Android 앱 · 음성 조리 기능 구현<br>Google Calendar 연동 및 알림 Agent 설계 | [![EJ-pro](https://img.shields.io/badge/EJ--pro-181717?logo=github&logoColor=white)](https://github.com/EJ-pro) |
...
행동 데이터를 기반으로 사용자별 추천 기능을 강화할 계획입니다.

- 저장, 선택, 조회, 소비 행동 데이터를 축적하여 개인화 추천 모델 확장
- `Recall@20`, `NDCG@20`을 활용한 추천 정확도 검증
- 클릭률, 저장률, 조리 완료율 등 사용자 반응 지표 분석

#### 2. 음성 AI 평가

주방 환경에서도 음성 기능이 안정적으로 동작하는지 검증할 계획입니다.

- 소음과 실제 조리 상황을 반영한 음성 질문 및 안내 테스트
- STT 정확도와 의도 분류 정확도 측정
- Task 성공률과 음성 응답 지연 시간 개선

#### 3. 서비스 효과 검증

추천 결과가 실제 식재료 소비와 조리 행동으로 이어지는지 확인할 계획입니다.

- GA4 사용량을 넘어 소비, 조리, 재방문 전환 효과 측정
- 소비 기간 내 식재료 소비율 분석
- 조리 전환율, 재방문율, 장보기 전환율 추적

#### 4. 모바일 사용성 개선

식재료 등록부터 메뉴 추천과 조리까지 이어지는 모바일 이용 과정을 단순화할 계획입니다.

- `재료 등록 → 메뉴 추천 → 조리` 과정의 이탈 구간 분석
- 단계별 과업 시간과 이탈률 측정
- 오류 발생률과 사용자 만족도를 기반으로 UX 개선

실사용 데이터를 지속적으로 축적하여 추천 정확도, 음성 기능의 사용성, 서비스 효과를 꾸준히 개선해 나갈 계획입니다.
...
<br>
약 2개월 동안 밥벌이 프로젝트를 진행하며 아이디어 구상부터 서비스 기획, 데이터 수집 및 전처리, AI 기능 개발, 웹 구현, 배포까지 하나의 서비스를 완성하는 전 과정을 경험했습니다. 단순히 기능을 개별적으로 구현하는 데 그치지 않고, 영수증 OCR을 통한 식재료 등록, 냉장고 재고 관리, 보유 재료 기반 레시피 추천, 부족 재료 장보기, 일정 등록까지 사용자의 식생활 흐름을 하나의 서비스로 연결했습니다.
<br>
프로젝트를 진행하면서 FastAPI, React, PostgreSQL, Neo4j, LangGraph, MCP, OCR 등 다양한 기술을 실제 서비스 구조 안에서 활용했습니다. 서로 다른 기술을 연결하고, 사용자 요청이 자연스럽게 이어지도록 데이터와 기능의 흐름을 설계하면서 단순한 기술 사용을 넘어 서비스 관점에서 개발하는 방법을 배울 수 있었습니다.
<br>
또한 팀원들과 역할을 나누고 기능 간 연동 문제를 함께 해결하며 협업 방식도 익혔습니다. 일정 안에서 우선순위를 조정하고, 반복적인 테스트와 개선을 거쳐 실제로 동작하는 결과물을 완성했다는 점이 가장 큰 성과였습니다. 2개월이라는 기간 동안 기술적 역량뿐 아니라 문제 해결, 협업, 일정 관리, 서비스 완성도에 대한 시각까지 폭넓게 성장할 수 있었던 프로젝트였습니다.
</details>
...
<br>
기획 단계에서는 여러 주제를 검토하고 변경하는 과정을 거쳤습니다. 처음에는 매력적으로 보였던 주제도 구체적으로 조사해보니 필수 사용 기술을 충분히 활용하기 어렵거나, 제한된 기간 안에 서비스로 구현하기에는 범위가 적절하지 않은 경우가 있었습니다. 이를 통해 주제 선정에서는 아이디어의 참신함뿐 아니라 기술 적합성, 데이터 확보 가능성, 구현 범위와 사용자 가치까지 함께 검토해야 한다는 점을 배웠습니다.
<br>
최종 주제를 확정한 뒤에는 한정된 개발 기간을 효율적으로 활용하기 위해 핵심 기능 구현에 집중했고, 계획한 일정보다 빠르게 주요 기능을 완성했습니다. 이를 통해 확보한 기간 동안 실제 사용 과정에서 문제와 개선점을 발견하고, 사용자 의도 분류, 멀티에이전트 실행 구조, 답변 품질 평가와 예외 처리 등 서비스 고도화에 집중할 수 있었습니다.
<br>
또한 테스트와 Langfuse 추적을 통해 정상적인 답변처럼 보여도 잘못된 Agent가 호출되거나 일부 작업이 누락될 수 있다는 점을 확인했습니다. 이번 프로젝트를 통해 AI 서비스는 단순히 모델을 연결하는 것이 아니라, 안정적인 라우팅과 상태 관리, 안전한 도구 실행, 지속적인 평가 체계를 함께 설계해야 한다는 것을 배웠습니다.
</details>
...
<br>
작업들 중 가장 인상 깊은 작업이 ML이었습니다. 추천 시스템에 사용할 모델을 개발해야 했는데 처음에는 어떻게 접근해야 할지 몰라서 시행착오가 많았습니다. 강의에서는 구현 목적과 맞는 데이터 셋을 가지고 시작하니 결과물도 명확하게 나오는 반면 우리가 수집한 데이터는 목적에 맞지 않거나 추론 결과가 잘 나오지 않는 데이터였기 때문입니다. 
<br>
실제 우리가 마주치는 데이터들은 원하는 결과를 내기에 부적합한 것들이 많을 거라고 생각합니다. 이 때 어떤 식으로 생각하고 작업해야 하는지 고민해 볼 수 있는 시간이었다고 생각합니다. 
</details>
