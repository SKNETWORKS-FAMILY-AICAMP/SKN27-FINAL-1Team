...
│  │  │  ├─ guide/                  (식재료 가이드 담당 / 보관·손질·신선도 API)
...
│  │     ├─ guide_service/          (식재료 가이드 담당 / 가이드 조회 로직)
...
│  ├─ api/                          (공용 / API 명세서)
│  ├─ erd/                          (공용 / RDB 테이블 설계)
│  ├─ graph_schema/                 (김경수·식재료 가이드 담당 / Neo4j 그래프 구조 설계)
...
│  ├─ food_guide/                   (식재료 가이드 담당 / 가이드 데이터 수집·검수)
│  │  ├─ collection/                (식재료 가이드 담당 / 보관·손질·세척 정보 수집)
│  │  ├─ validation/                (식재료 가이드 담당 / 수집 데이터 검수)
│  │  └─ load_to_neo4j/             (식재료 가이드 담당 / 가이드 데이터 Neo4j 적재)
│  └─ receipt_samples/              (박준희 담당 / OCR 테스트용 영수증 샘플 관리)
│
├─ storage/                         (원본·가공 데이터와 Neo4j 파일 보관 공간)
│  ├─ raw/                          (원본 데이터 보관)
│  │  ├─ recipe/                    (김경수 담당 / 원본 레시피 데이터)
│  │  ├─ food_guide/                (식재료 가이드 담당 / 원본 가이드 수집 데이터)
│  │  └─ receipts/                  (박준희 담당 / 원본 영수증 이미지·텍스트)
│  ├─ processed/                    (전처리 완료 데이터 보관)
│  │  ├─ recipe/                    (김경수 담당 / 전처리된 레시피 데이터)
│  │  ├─ food_guide/                (식재료 가이드 담당 / 검수된 가이드 데이터)
│  │  └─ receipts/                  (박준희 담당 / 구조화된 OCR 결과)
│  ├─ neo4j/                        (Neo4j 적재/백업 관련 파일)
│  │  ├─ import/                    (김경수·식재료 가이드 담당 / Neo4j import CSV)
│  │  ├─ cypher/                    (김경수·식재료 가이드 담당 / Cypher 쿼리)
...
│  │  └─ food_guide/                (식재료 가이드 담당 / 가이드 테스트 샘플)
