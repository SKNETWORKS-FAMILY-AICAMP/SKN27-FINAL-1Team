...
> _이재희_ :  
> _박준희_ :  
> _김재묵_ :  기획 단계에서는 여러 주제를 검토하고 변경하는 과정을 거쳤습니다. 처음에는 매력적으로 보였던 주제도 구체적으로 조사해보니 필수 사용 기술을 충분히 활용하기 어렵거나, 제한된 기간 안에 서비스로 구현하기에는 범위가 적절하지 않은 경우가 있었습니다. 이를 통해 주제 선정에서는 아이디어의 참신함뿐 아니라 기술 적합성, 데이터 확보 가능성, 구현 범위와 사용자 가치까지 함께 검토해야 한다는 점을 배웠습니다.
...
> _김주영_ : 공공기관의 식재료 가이드 데이터를 수집·전처리하고, 기관마다 다른 식재료명을 표준명과 별칭으로 정리해 PostgreSQL과 Neo4j에 적재했습니다. 구축한 데이터는 식재료 가이드 에이전트와 연결해 보관법, 손질법, 세척법, 신선도, 제철, 영양정보를 제공하도록 구현했으며, GA4를 연동해 사용자 이용 흐름을 분석할 수 있는 기반도 마련했습니다.  
> 또한 동일한 데이터를 기준으로 PostgreSQL과 Neo4j의 조회 성능을 비교했습니다. 0~1-Hop 단순 조회는 PostgreSQL이 우세했지만, `레시피 → 식재료 → 가이드 → 출처`와 같은 3-Hop 관계 조회에서는 PostgreSQL 11.911ms, Neo4j 7.120ms로 Neo4j의 응답 시간이 약 40.2% 낮았습니다. 이를 통해 단순 조회와 CRUD는 PostgreSQL, 다단계 관계 탐색은 Neo4j가 적합하다는 결론을 도출했습니다.  
> 이번 프로젝트를 통해 데이터 수집·전처리·적재뿐 아니라, 실제 성능 비교를 바탕으로 데이터베이스의 역할을 결정하고 서비스 구조에 반영하는 경험을 했습니다. 향후에는 두 저장소의 동기화 기준과 검수 이력, Neo4j 백업·복구 체계를 보완하고, GA4 데이터를 활용해 식재료 가이드 에이전트의 이용 흐름과 응답 품질을 지속적으로 개선하고자 합니다.
>
> _김경수_ :
