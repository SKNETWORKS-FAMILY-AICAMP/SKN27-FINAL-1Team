...
공식 문안의 “Warm Dataset Spearman ≥ 0.30”은 **순위가 성립하는 warm 구간(informative subset, n≈71)** 에 적용한다. warm 563개 중 약 94%는 별점 천장(ceiling)으로 관측 bar 분산이 없어 Spearman이 정의상 무의미하다(§[experiments.md](experiments.md) 실험 14). **전체 warm Spearman**은 범용 목표(Cohen 0.30)를 유지하되 **Go에서 제외(L4, DATASET_EXCEPTION)** 하고 calibration 수치만 기록한다.
...
| **L1** | Anti-Random | ρ > 0.10, null permutation p < 0.05; popularity 대비 **4/5 seed** 우위 | 랜덤·인기-only 대비 우수 |
| **L2** | Ranking Quality | warm **informative** Spearman **≥ 0.30** | Cohen Medium; 순위 평가 가능 구간 |
...
**L1 multi-seed:** 단일 run은 `l1_single_pass`(ρ·null·pop 1회 비교). 최종 `L1_pass`는 **5 seed** calibration에서 popularity 대비 **4/5 wins**로 확정 ([실험 11](experiments.md) 패턴).
...
| baseline | 정의 |
|----------|------|
| **null_rank** | `ŷ` 무작위 순열 (1000회, seed 고정) |
| **popularity_rank** | `log1p(view_count) + log1p(scrap_count)` |

| 조건 | 임계값 | 근거 |
|------|--------|------|
| 모델 ρ | **> 0.10** | Cohen small |
| null 대비 | permutation **p < 0.05** | 통계적 유의 |
| popularity 대비 | **4/5 seed**에서 ρ(model) > ρ(pop) | [실험 11·12](experiments.md) |
...
| 실측 상한 (거친) | 529×0 + 71×0.5 / 563 ≈ **0.06** |
