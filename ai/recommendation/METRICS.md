...
1. **정확한 점수 계산은 필수가 아니다** — Base Score는 regression target이 아니라 **상대 순위를 실을 용기**다.  
2. **상대 순위를 얼마나 유의미하게 나누는가**가 개선의 척도다 (동점·천장 붕괴를 줄이는 것).  
3. **점수의 절대 가치(스케일·오프셋)는 부차** — 순서가 맞으면 서비스에 충분하고, MAE/MinMax만으로 “맞춘다”고 보지 않는다.

Go·실험 판단은 Spearman·baseline 대비·슬라이스 구분력을 우선한다 ([실험 20](experiments.md) 인사이트).
...
| **bar** | Bayesian WR on `mean(star_02×sentiment_02)` (m=3) | warm 관측 정답; `BAR_MODE=mean` 시 단순 mean |
...
**해석 범위 (실험 19~20):** Go는 informative에서 공정 축으로 확인. **bar**는 실험 20부터 Bayesian WR(m=3) on product mean ([실험 20](experiments.md)). ceiling ρ는 개선됐으나 v=1 all5 동점은 잔여.
...
| 2026-07-14 | **20** | bar = Bayesian WR (m=3); ceiling ρ↑; Go 유지; **순위>절대점수** 원칙 문서화 |
