...
### 빌드 보조 패키지 (Python 3.13 / Windows 대응)
- `cython`
- `setuptools`
- `wheel`
...
`uv pip install -r ai/experiments/requirements.txt`에서 `lightfm==1.17` 빌드가 실패할 경우,
아래 순서로 빌드 도구 버전을 먼저 고정한 뒤 재시도한다.
...
이후 설치 재시도:
```powershell
uv pip install -r ai\experiments\requirements.txt
```

참고:
- 위 단계 통과 후 `Microsoft Visual C++ 14.0 or greater is required`가 발생하면
  C++ Build Tools 설치가 추가로 필요하다.
