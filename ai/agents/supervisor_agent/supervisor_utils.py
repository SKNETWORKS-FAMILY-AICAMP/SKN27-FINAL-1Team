...
def _build_read_tasks(text: str) -> list[dict[str, str]]:
    ...
    if any(phrase in normalized for phrase in _GUIDE_PHRASES if phrase != "먹다남은"):
...
def _parse_llm_route_payload(content: str) -> dict[str, Any]:
    ...
    for task in payload.get("tasks") or []:
        ...
        if intent not in _MULTI_AGENT_TASK_INTENTS:
            continue
        tasks.append({"intent": intent, "text": str(task.get("text", "")).strip()})
    return {
...
def _merge_agent_results(*results: dict[str, Any]) -> dict[str, Any]:
    """여러 Agent 응답의 문장과 UI 데이터를 하나의 GraphState 응답으로 합칩니다."""
    response_text = "\n\n".join(
        result.get("response_text", "").strip()
        for result in results
        if result.get("response_text", "").strip()
    )
    actions = [action for result in results for action in result.get("actions") or []]
