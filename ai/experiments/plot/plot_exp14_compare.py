"""Grouped bar chart: subset x target Spearman vs review_rank_score."""
...
def collect_long_metrics() -> pd.DataFrame:
    ...
    for target in TARGETS:
        csv_path = EXPERIMENTS_ROOT / f"recipe_lightfm_exp14_{target}.csv"
        json_path = EXPERIMENTS_ROOT / "runs" / f"exp14_{target}.json"
        if json_path.exists():
        ...
        for row in decomposed_metrics_from_csv(csv_path):
            rows.append({"target": target, **row})
    if not rows:
        raise SystemExit("no exp14 metrics found (run training + plot_decompose first)")
...
def main() -> None:
    ...
    out_csv = FIGURES_DIR / "exp14_target_subset_metrics.csv"
    ...
    ax.set_title("Experiment 14 — target x subset (review_rank bar)")
    ax.legend(title="TARGET_MODE", fontsize=8)
    fig.tight_layout()

    out_png = FIGURES_DIR / "exp14_target_subset_compare.png"
