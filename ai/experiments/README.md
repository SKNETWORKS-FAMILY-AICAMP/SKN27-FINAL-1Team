...
- 데이터: `review_by_llm.csv` / `recipe_fix.csv` / `recipe_ingredient_alias.csv`.
...
통과: exit 0, `runs/ablation_report.json` 생성, `recipe_lightfm.csv` (3,171행), `decision`에 `track_b_eval` 기준.
...
| 7 | LightFM 학습 |
| 11 | catalog predict → `recipe_lightfm.csv` + `catalog_eval` |
...
| 파일 | 역할 |
|------|------|
| `LightFM_Model.ipynb` | 유일한 실행 파이프라인 (커밋) |
...
| `score_02.py` | 0~2 스케일 (`star_02`, `sentiment_02`, row product) |
| `recipe_lightfm.csv` | 전 카탈로그 Base Score export |
| `runs/` | 임시 JSON (비커밋 권장) |
...
- **실험 17:** Track B 콜드스타트 재정의 — full train, CF/baseline dead code 제거, `product_02_row` default, B2 Spearman-only Go. `figures/`·`samples/`·`plot/`·`baseline_eval.py`·`bar_eval.py` 폐기. B0·B3 통과, B2 미달 (ρ=-0.052).
