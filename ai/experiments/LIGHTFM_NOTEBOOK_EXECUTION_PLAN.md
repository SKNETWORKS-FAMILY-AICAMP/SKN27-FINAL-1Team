...
### 빌드 보조 패키지 (Python 3.11 / Windows 대응)
- `cython`
- `setuptools`
- `wheel`
...
### Windows 설치 트러블슈팅 (LightFM 1.17)

실측 기준으로 `lightfm==1.17`은 Python 3.12/3.13 + Windows에서 빌드 실패 가능성이 높다.
실험 환경은 `ai/experiments` 로컬 venv + Python 3.11을 기본으로 사용한다.
...
위 명령은 다음 목적을 가진다.
- `pip<25`: legacy 빌드 경로 호환성 확보 (`--no-use-pep517` 사용 시)
- `setuptools==69.5.1`: LightFM setup.py와 충돌 빈도가 낮은 버전으로 고정
- `wheel==0.43.0`: 빌드 휠 생성 경로 안정화
- `cython==3.0.11`: C 확장 빌드 의존성 명시

LightFM은 `python -m pip`로 직접 설치한다. (`uv pip` 사용 시 `__LIGHTFM_SETUP__` 오류 재발 가능)
```powershell
...
python -c "from lightfm import LightFM; print('lightfm import ok')"
```

실패 분기:
- `__LIGHTFM_SETUP__` 오류: `uv pip` 대신 `python -m pip`로 재시도
- `Microsoft Visual C++ 14.0 or greater is required`: C++ Build Tools 설치 필요
- `pytz`/`dateutil` 누락: `--no-deps` 설치 이후 발생한 누락이므로 `python -m pip install -r .\requirements.txt` 재실행
