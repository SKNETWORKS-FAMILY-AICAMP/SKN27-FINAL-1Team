...
목표 분류 체계는 4개 대분류와 35개 중분류로 고정한다.
...
- `nodes_middle_category.csv`에는 위 35개 중분류만 저장한다.
- 식재료는 하나의 중분류에만 연결한다.
- 가공 여부를 우선 적용해 육가공품과 수산가공품은 각각 `육가공품류`, `수산가공품류`로 분류한다.
- 현재 분할 CSV의 기존 26개 중분류 수치는 재분류 완료 전의 현행 데이터 검증값이다.
