...
Guide Agent는 단일 질문을 받아 식재료 가이드 응답을 생성하는 모듈이다.

Guide Agent가 책임지는 범위:

- 식재료명 전처리
- 가이드 유형 분류
- Neo4j / PostgreSQL / Web fallback 조회
- fuzzy 후보 제시
- 공통 JSON 응답 생성

Guide Agent가 책임지지 않는 범위:

- `/api/v1/chat`에서 어떤 Agent로 보낼지 결정
- 후보 선택 후 이전 대화 의도를 저장하고 복원
- 사용자가 후보 버튼 대신 `닭가슴살`처럼 식재료명만 직접 입력했을 때 이전 guide type 복원
- 여러 Agent 결과를 합쳐 최종 대화 흐름을 조율

위 항목은 Supervisor, 라우터, 프론트의 대화 상태 관리 범위다.
...
| `고추 냉동해도 괜찮아?` | `고추` |
| `고추가 물러졌는데 괜찮아?` | `고추` |
| `닭고기 상했는지 확인하는 법 알려줘` | `닭고기` |
| `닭고기가 상한 것 같아 먹어도 돼?` | `닭고기` |
| `고추 단백질 얼마나 들어 있어?` | `고추` |
...
후보 선택 액션에는 기존 프론트/라우터가 사용할 수 있도록 `value`와 `data.message`를 함께 담는다.
...
```json
{
  ...
  "data": {
    "message": "닭가슴살 신선도 확인법 알려줘"
  },
  "intent": "ingredient.guide",
  "guide_type": "freshness",
  "original_query": "닭고기 상했는지 확인하는 법 알려줘"
}
```

다만 사용자가 후보 버튼을 누르지 않고 직접 `닭가슴살`만 입력하면 이전 guide type은 복원할 수 없다. 이 경우는 Guide Agent가 아니라 대화 상태를 가진 라우터/Supervisor가 처리해야 한다.

2글자 한글 오타는 같은 첫 글자와 같은 초성의 두 번째 글자를 가진 경우 보수적으로 유사 후보로 인정한다.

```text
감쟈 → 감자
```

이 규칙은 한글 2글자에만 적용되며, 영문/숫자 2글자에는 적용하지 않는다.
...
| `닭고기 상했는지 확인하는 법 알려줘` | keyword=`닭고기`, 신선도 후보 제시 |
...
| `감쟈 보관법` | `감자`로 fuzzy 자동 보정 후 보관법 조회 |
...
- `딸기 5월에 제철이야?` 전처리
- `고추 냉동해도 괜찮아?` 전처리
- `닭고기 상했는지 확인하는 법 알려줘` 전처리
- `닭고기가 상한 것 같아 먹어도 돼?` 전처리
...
- 2글자 한글 오타 fuzzy 보정
- 후보 선택 응답에 `value`와 `data.message` 동시 제공
- 영양성분 부분 일치 오조회 방지
...
Guide Agent 직접 호출 기준으로 아래 질문은 정상 동작한다.

| 질문 | 기대 결과 |
|---|---|
| `감자 알려줘` | `success / lookup_ingredient` |
| `감자 분류정보는?` | `success / list_related_ingredients` |
| `고추가 물러졌는데 괜찮아?` | `success / lookup_freshness` |
| `감쟈 보관법 알려줘` | `success / lookup_storage`, `감자` 자동 보정 |
| `닭고기 상했는지 확인하는 법 알려줘` | `needs_input / confirm_ingredient`, 신선도 후보 제시 |
| `고추 영양성분 알려줘` | `success / lookup_nutrition` |
| `제철 식재료 알려줘` | `needs_input / request_season_month` |
| `13월 제철 식재료 알려줘` | `needs_input / request_season_month` |
| `어떤 식재료가 있어?` | `needs_input / list_related_ingredients` |

웹 챗봇 화면에서 `음식과 관련된 대화만 지원하고 있어요` 또는 `올바른 식재료명을 입력해주세요`가 먼저 반환되면 Guide Agent까지 도달하지 않은 것이다. 이 경우는 Guide Agent 내부 문제가 아니라 라우터/Supervisor의 도메인 판별 또는 Agent 선택 문제다.
