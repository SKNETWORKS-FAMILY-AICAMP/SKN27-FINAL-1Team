...
분류 구조는 4개 대분류와 25개 중분류를 사용합니다. 전체 목록과 분류 규칙은
`docs/food_guide_neo4j.md`의 `분류 체계`를 단일 기준으로 사용합니다.
...
원재료명 결정 우선순위는 다음 기준으로 처리했다.

| 우선순위 | 기준 |
| ---: | --- |
| 1순위 | 식품DB `영양식품명` |
| 2순위 | 서비스에서 쓰기 쉬운 대표 원재료명 |
| 3순위 | 기존 원재료명은 Alias로 보존 |

영양DB는 원재료성 식품 DB를 우선 사용하고, 부족한 경우에만 식약처 영양성분 DB로 보완했다.

| 파일 | 총 건수 | 구분 | 사용 컬럼 | 분류 개수 | 처리 기준 |
| --- | ---: | --- | --- | ---: | --- |
| 전국통합식품영양성분정보_원재료성식품.csv | 3,704 | 대분류 | `typeNm` | 1 | `typeNm`은 모두 원재료성 식품이라 실제 서비스 분류로는 너무 넓다. |
| 전국통합식품영양성분정보_원재료성식품.csv | 3,704 | 중분류 | `foodLv3Nm` | 17 | 실무 기준에서는 대분류처럼 활용하는 것이 자연스럽다. |
| 전국통합식품영양성분정보_원재료성식품.csv | 3,704 | 소분류 | `foodLv4Nm` | 699 | 원재료 세부 분류 참고용으로 사용한다. |
| 전국통합식품영양성분정보_원재료성식품.csv | 3,704 | 세분류 | `foodLv5Nm` | 934 | 원재료 세부 분류 참고용으로 사용한다. |
| 식품의약품안전처_식품영양성분DB정보.csv | 302,628 | 대분류 | `FOOD_CAT1_NM` | 64 | 음식, 가공식품, 원재료가 섞여 있어 보완용으로 사용한다. |
| 식품의약품안전처_식품영양성분DB정보.csv | 302,628 | 중분류 | `FOOD_CAT2_NM` | 1,271 | 음식, 가공식품, 원재료가 섞여 있어 보완용으로 사용한다. |
| 식품의약품안전처_식품영양성분DB정보.csv | 302,628 | 소분류 | `FOOD_CAT3_NM` | 460 | 음식, 가공식품, 원재료가 섞여 있어 보완용으로 사용한다. |
| 식품의약품안전처_식품영양성분DB정보.csv | 302,628 | 세분류 | `FOOD_CAT4_NM` | 75 | 음식, 가공식품, 원재료가 섞여 있어 보완용으로 사용한다. |
| 식품의약품안전처_식품영양성분DB정보.csv | 302,628 | 대표식품명 | `FOOD_REF_NM` | 2,126 | 대표 원재료명 후보와 영양정보 보완 기준으로 사용한다. |
